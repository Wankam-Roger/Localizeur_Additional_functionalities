...
@section('page_title','SHOW subscriberS')
                        ...
                        <div id="tagList">
                            <div class="row g-4 mb-3">
                                <div class="col-sm-auto">
                                    <div>
                                        <a type="button"
                                           href="{{ route('admin_create_book_subscriber_page',[app()->getLocale()]) }}"
                                           class="btn btn-primary bg-animation w-lg waves-effect waves-light d-grid">Ajouter
                                            un Tag</a>
                            ...
                            <div class="table-responsive table-card mt-3 mb-1">
                                <table class="table align-middle table-nowrap" id="tagTable">
                                    ...
                                    <tr>
                                        ...
                                        <th class="sort" data-sort="added_by">Telephone</th>
                                        <th class="sort" data-sort="book">Livre</th>
                                    ...
                                    @foreach ($book_subscribers as $subscriber)
                                        <tr>

                                            <td class="id" style="display:none;"><a href="javascript:void(0);"
                                                                                    class="fw-medium link-primary">{{ $subscriber->id }}</a>
                                            </td>
                                            <td class="name">{{ $subscriber->name }}</td>
                                            <td class="email">{{ $subscriber->email }}</td>
                                            <td class="phonenumber">{{ $subscriber->phonenumber }}</td>
                                            <td class="book">{{ $subscriber->book ? $subscriber->book->title_fr : '' }}</td>
                                            <td class="created_at">{{ date('d-m-Y', strtotime($subscriber->created_at)) }}</td>
                                            <td>
                                                <div class="d-flex gap-2">
                                                    <div class="edit">
                                                        <button class="btn btn-sm btn-success edit-item-btn"
                                                                data-bs-toggle="modal" data-bs-target="#edittagModal"
                                                                data-bs-tag-id="{{ $subscriber->id }}"
                                                                data-bs-tag-des="{{ $subscriber->email }}"
                                                                data-bs-tag-name="{{ $subscriber->name }}">Modifier
                                                        </button>
                                                    </div>
                                                    <div class="remove">
                                                        <button class="btn btn-sm btn-danger remove-item-btn"
                                                                data-bs-toggle="modal" data-bs-target="#deletetagModal"
                                                                data-bs-tag-id="{{ $subscriber->id }}">Supprimer
                                                        </button>
                                                    </div>
                                                </div>
                                            </td>
    ...
    <div class="modal fade" id="edittagModal" tabindex="-1"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form method="post" action="{{ route('admin_update_book_subscriber_page',[app()->getLocale()]) }}"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3" id="modal-id" style="display: none;">
                            ...
                            <input aria-readonly="true" type="text" id="id-field" name="id" class="form-control"
                                   placeholder="ID" readonly/>
                        </div>

                        <div class="mb-3">
                            <label for="name-field" class="form-label">Nom du Tag</label>
                            <input type="text" id="name-field" name="name_fr" class="form-control"
                                   placeholder="Enter Name" required/>
                        </div>

                        <div class="mb-3">
                            <label for="description-field" class="form-label">description</label>
                            <textarea id="description-field" name="short_description_fr" rows="5" class="form-control"
                ...
                <div class="modal-body">
                    <div class="mt-2 text-center">
                        ...
                        <div class="mt-4 pt-2 fs-15 mx-4 mx-sm-5">
                            <h4>Are you Sure ?</h4>
                            <p class="text-muted mx-4 mb-0">Voulez vous vraiment supprimer cette categorie?<br> Cette
                                action est irreversible</p>
                        </div>
                    </div>
                    <form method="post" action="{{ route('admin_delete_book_subscriber_page',[app()->getLocale()]) }}"
                          enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3" id="modal-id" style="display: none;">
                            <label for="id-field" class="form-label">ID</label>
                            <input aria-readonly="true" type="text" id="id-field-delete" name="id" class="form-control"
                                   placeholder="ID" readonly/>
                        </div>
                        <div class="d-flex gap-2 justify-content-center mt-4 mb-2">
                            <button type="button" class="btn w-sm btn-light" data-bs-dismiss="modal">Fermer</button>
                            <button type="submit" class="btn w-sm btn-danger " id="delete-record">Oui, Supprimer
                            </button>
                        </div>
                    </form>
    ...
    <script type="text/javascript">
        ...
        edittagModal.addEventListener("show.bs.modal", function (t) {
            var id = t.relatedTarget.getAttribute("data-bs-tag-id"),
                name = t.relatedTarget.getAttribute("data-bs-tag-name");
